<?php
namespace NethServer\Module\FlashStart\SrcProfile;
...
class Modify extends \Nethgui\Controller\Table\Modify
{
    ...
    public function prepareView(\Nethgui\View\ViewInterface $view)
    {
        ...
        $host_group = $this->getPlatform()->getDatabase('hosts')->getAll('host-group');

        $tmp = array();
        foreach($host_group as $key => $props) {
    ...
    public function process()
    {
        if ($this->getIdentifier() === 'create' && $this->getRequest()->isMutation()) {
            ...
            $this->getPlatform()->getDatabase('flashstart')->setKey($this->parameters['Group'], 'src-profile',
    ...
    public function nextPath()
    {
        ...
        if($this->getRequest()->isMutation()) {
            return '/FlashStart/SrcProfile/read';
        }
